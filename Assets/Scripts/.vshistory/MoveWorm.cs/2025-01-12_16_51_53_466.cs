...
public class MoveWorm : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.LeftArrow) && laneIndex > -1 && !Physics.Raycast(transform.position, Vector3.left, 5))
        {
            laneIndex--;
            changeLane = true;
        } else if (Input.GetKeyDown(KeyCode.RightArrow) && laneIndex < 1 && !Physics.Raycast(transform.position, Vector3.right, 5)) {
            laneIndex++;
            changeLane = true;
        }

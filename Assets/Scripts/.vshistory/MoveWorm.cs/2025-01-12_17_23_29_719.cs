...
public class MoveWorm : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Vector3 rayposLD = new Vector3(transform.position.x -5, transform.position.y, transform.position.z - );
        if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out RaycastHit hitInfoLD, 5))
        {
            Debug.DrawRay(transform.position, transform.TransformDirection(Vector3.left) * hitInfoLD.distance, Color.red);
        }
        else
        {
            Debug.DrawRay(transform.position, transform.TransformDirection(Vector3.left) * 5, Color.green);
        }
        ...
        {
            Debug.DrawRay(transform.position, transform.TransformDirection(Vector3.right) * 5, Color.green);
        }

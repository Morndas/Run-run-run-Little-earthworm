...
public class Move : MonoBehaviour
{
    ...
    IEnumerator MoveForward()
    {
        while (true)
        {
            if (!isPathObstructed)
            {
                // Accelere graduellement jusqu'à la vitesse max
                currentForwardSpeed += .1f * Time.deltaTime;
                ...
                transform.parent.transform.Translate(currentForwardSpeed * Vector3.forward);
            }

...
public class MoveWorm : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.LeftArrow) && laneIndex > -1)
        {
            if (!Physics.Raycast(rayPosL, transform.TransformDirection(Vector3.forward), out RaycastHit hitInfoL, wormCollider.bounds.size.z))
            {
                ...
                StartCoroutine(SwitchLane(laneIndex, --laneIndex, .3f));
            }
            else
            {
    ...
    private IEnumerator SwitchLane(int originLineIndex, int targetLineIndex, float duration)
    {
        float timeElapsed = 0;
        while (timeElapsed < duration)
        {
            Vector3 newPos = new Vector3(transform.localPosition.y, transform.localPosition.y, transform.localPosition.z);
